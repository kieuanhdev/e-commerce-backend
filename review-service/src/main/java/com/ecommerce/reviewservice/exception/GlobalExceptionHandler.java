...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ResponseError> handleRuntimeException(RuntimeException ex) {
        return new ResponseEntity<>(new ResponseError(HttpStatus.BAD_REQUEST.value(), ex.getMessage()), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ResponseError> handleException(Exception ex) {
        return new ResponseEntity<>(new ResponseError(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Lỗi hệ thống: " + ex.getMessage()), HttpStatus.INTERNAL_SERVER_ERROR);
    }
}